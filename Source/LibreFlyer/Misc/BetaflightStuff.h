...
struct FBetaflightRates {
	...
	*/
	...
	void Apply(int axis, float& rcCommandf, const float& rcCommandfAbs) {
		if (rcCommandfAbs > EasyRates.Deadzone) {
			rcCommandf = copysignf(1.f, rcCommandf) * ((rcCommandfAbs - EasyRates.Deadzone) / (1.f - EasyRates.Deadzone));
		}
		else {
			rcCommandf = 0.f;
		}
		...
		else if (RatesType == ERatesType::Easy) {
			rcCommandf *= EasyRates.MaxRotationSpeed * (EasyRates.CenterStickPrecision * powf(rcCommandfAbs, 20.f * powf(EasyRates.OuterStickTransition, 2.f)) + (1.f - EasyRates.CenterStickPrecision));
		}
	};
};
